...
def _batch_viewdir_to_yaw_pitch(viewdirs):
    """
    Args:
        viewdirs: np.ndarray of shape (N, 3), 每行是一个 viewdir 单位向量 [vx, vy, vz]
    Returns:
        yaw_list:   shape (N,)，每个 viewdir 的 yaw（度）
        pitch_list: shape (N,)，每个 viewdir 的 pitch（度）
    """
    viewdirs = np.array(viewdirs)
    normed = viewdirs / np.linalg.norm(viewdirs, axis=1, keepdims=True)

    vx, vy, vz = normed[:, 0], normed[:, 1], normed[:, 2]
    yaw = np.degrees(np.arctan2(vy, vx))        # [-180, 180]
    pitch = np.degrees(np.arcsin(-vz))          # [-90, 90]
...
def get_google_earth_aligned_seg_maps(
    ...
    for gcp in tqdm(ge_camera_poses["poses"], desc="Project: %s" % ge_project_name):
        ...
        breakpoint()
        yaw, pitch = viewdir_to_yaw_pitch(viewdir.cpu())
        # breakpoint()
        viewdir = yaw_pitch_to_viewdir(yaw, pitch)  # DJI pitch = -pitch  DJI yaw = 180 - yaw
        # viewdir = recover_viewdir(viewdir, gcp["position"]["z"])
        viewdir = torch.from_numpy(viewdir).float().cuda()
        # breakpoint()
        (
            voxel_id,
            depth2,
        ...
        # pitch , yaw = _batch_viewdir_to_yaw_pitch(viewdir)
        if debug:
            # breakpoint()
            ...
            test = voxel_id.squeeze()[..., 0].cpu().numpy()
            test[test >= CONSTANTS["BLD_INS_LABEL_MIN"]] = CLASSES["BLD_FACADE"]
            breakpoint()
            seg_map = utils.helpers.get_seg_map(
                test
            )
    ...
    for of in tqdm(osm_files):
        ...
        for gep in ge_projects:
            ...
            for idx, sg in enumerate(seg_maps):
                ...
                if debug:
                    sg.save(os.path.join(_ges_out_dir, "%s-%02d.jpg" % (gep, idx)))  #jpg
                else:
                    with open(
...
def main_2(ges_dir, osm_dir, ges_out_dir, seg_dir, patch_size, zoom_level, max_height, debug):

    tensor_extruder = TensorExtruder(max_height)
    basename = osm_dir.split('/')[-1]
    ge_projects = get_google_earth_projects(basename, ges_dir)
    if not ge_projects:
        logging.warning(
            "No matching Google Earth Project found for OSM[File=%s]." % basename
        )
    # Rasterisation
    output_hf_file_path = os.path.join(osm_dir, "hf.png")
    # output_ctr_file_path = os.path.join(_osm_out_dir, "ctr.png")
    output_seg_map_file_path = os.path.join(osm_dir, "seg.png")
    metadata_file_path = os.path.join(osm_dir, "metadata.json")
    ...
    if (
        ...
        height_field, seg_map, contours, metadata = get_osm_images(
            os.path.join(osm_dir, basename), # Bug
            os.path.join(osm_dir, "tiles.png"),  # Bug
            zoom_level,
        )
        Image.fromarray(height_field).save(output_hf_file_path)
        # Image.fromarray(contours).save(output_ctr_file_path)
